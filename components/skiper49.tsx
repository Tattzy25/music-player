...
const Carousel_003 = ({
  ...
  autoplay = false,
  spaceBetween = 0,
  ariaLabel = "Featured artwork carousel",
}: {
  ...
  // AIMD: animation-performance + accessibility-basics (reduced motion)
  const [prefersReducedMotion, setPrefersReducedMotion] = useState(false);
  const [swiper, setSwiper] = useState<SwiperType | null>(null);

  useEffect(() => {
    const mm = (typeof globalThis !== "undefined" && "matchMedia" in globalThis
      ? (globalThis as unknown as Window).matchMedia
      : undefined);
    if (!mm) return;
    const mql = mm("(prefers-reduced-motion: reduce)");
      ...
      >
        <Swiper
          ...
          coverflowEffect={{
            ...
            depth: 100,
            modifier: 1,
            slideShadows: !prefersReducedMotion,
          }}
          pagination={
            showPagination
              ? {
        ...
        >
          {images.map((image) => (
            <SwiperSlide key={image.src} className="">
              <img
